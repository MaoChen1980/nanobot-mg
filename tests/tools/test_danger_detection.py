...
# ---------------------------------------------------------------------------
# EditFilesTool — stale file and destructive edit danger
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_edit_files_stale_file_warning(tmp_path):
    """EditFilesTool should warn when a file changed since last read."""
    from nanobot.agent.tools.edit_files import EditFilesTool
    from nanobot.agent.tools import file_state

    fp = tmp_path / "stale.txt"
    fp.write_text("original content\n")

    # Simulate a prior read so file_state has a hash
    file_state.record_read(fp)

    # Modify the file externally to make it stale
    fp.write_text("modified content\n")

    tool = EditFilesTool(allowed_dir=tmp_path)
    result = await tool.execute(edits=[
        {
            "path": str(fp),
            "action": "replace",
            "old_text": "modified content\n",
            "new_text": "new content\n",
        },
    ])
    assert result.startswith("⚠️ Danger:")
    assert "changed since last read" in result.lower() or "stale" in result.lower()


@pytest.mark.asyncio
async def test_edit_files_stale_file_override(tmp_path):
    """EditFilesTool should proceed with danger_override=true for stale files."""
    from nanobot.agent.tools.edit_files import EditFilesTool
    from nanobot.agent.tools import file_state
...
@pytest.mark.asyncio
async def test_edit_files_destructive_replace_warning(tmp_path):
    """EditFilesTool should warn when a replace operation deletes content without adding any."""
    from nanobot.agent.tools.edit_files import EditFilesTool

    fp = tmp_path / "target.txt"
    ...
    assert result.startswith("⚠️ Danger:")
    assert "removing" in result.lower() or "delet" in result.lower()


@pytest.mark.asyncio
async def test_edit_files_destructive_replace_override(tmp_path):
    """EditFilesTool should proceed with danger_override for destructive replace."""
    from nanobot.agent.tools.edit_files import EditFilesTool
