...
# ---------------------------------------------------------------------------
# _detect_user_corrections — unit tests for the static method
# ---------------------------------------------------------------------------


class TestDetectUserCorrections:
    """Unit tests for AgentRunner._detect_user_corrections static method."""
    # Import locally for consistency with existing test conventions
    from nanobot.agent.runner import AgentRunner

    # ── Positive detections ──

    def test_chinese_bu_dui_detected(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "不对，重新做"}]
        ) is True

    def test_chinese_bu_shi_detected(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "不是这个"}]
        ) is True

    def test_chinese_cuo_le_detected(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "错了"}]
        ) is True

    def test_chinese_bie_detected(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "别这样做"}]
        ) is True

    def test_chinese_ting_detected(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "停，不是这个"}]
        ) is True

    def test_english_wrong_detected(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "that's wrong"}]
        ) is True

    def test_english_incorrect_detected(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "incorrect answer"}]
        ) is True

    def test_english_not_what_i_detected(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "not what I meant"}]
        ) is True

    def test_english_you_misunderstood_detected(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "you misunderstood"}]
        ) is True

    def test_chinese_zhongxin_detected(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "重新开始"}]
        ) is True

    def test_chinese_chonglai_detected(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "重来一次"}]
        ) is True

    # ── No detection (neutral) ──
    ...
    def test_technical_question_returns_false(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "解释一下这个算法的工作原理"}]
        ) is False

    def test_empty_messages_returns_false(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections([]) is False
    ...
    def test_system_role_skipped(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "system", "content": "不对，但这是系统指令"}]
        ) is False

    def test_assistant_role_skipped(self):
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "assistant", "content": "不对，这是助手输出"}]
        ) is False

    # ── 20-message window ──

    def test_correction_beyond_20_window_ignored(self):
        msgs = [{"role": "user", "content": f"msg{i}"} for i in range(21)]
        msgs[0] = {"role": "user", "content": "不对"}
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(msgs) is False

    def test_correction_within_20_window_detected(self):
        msgs = [{"role": "user", "content": f"msg{i}"} for i in range(19)]
        msgs.append({"role": "user", "content": "不对，修正一下"})
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(msgs) is True

    def test_correction_at_edge_20th_position_detected(self):
        msgs = [{"role": "user", "content": f"msg{i}"} for i in range(20)]
        msgs[-1] = {"role": "user", "content": "不要这样"}
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(msgs) is True

    # ── Known false positives (documenting current behavior) ──

    def test_false_positive_bie_in_fenbie(self):
        """Known: '别' in '分别' is a substring match false positive."""
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "请分别处理这两个文件"}]
        ) is True

    def test_false_positive_ting_in_tingzhi(self):
        """Known: '停' is a substring match that can false-positive."""
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(
            [{"role": "user", "content": "停电了怎么办"}]
        ) is True

    # ── Multiple messages ──

    def test_correction_in_second_message(self):
        msgs = [
            {"role": "user", "content": "你好"},
            {"role": "assistant", "content": "我来帮你"},
            {"role": "user", "content": "搞错了，重做"},
        ]
        assert TestDetectUserCorrections.AgentRunner._detect_user_corrections(msgs) is True

    def test_no_correction_mixed_roles(self):
        msgs = [
            {"role": "system", "content": "不对"},  # system, skipped
            {"role": "user", "content": "你好"},
...
# ---------------------------------------------------------------------------
# _detect_user_corrections — runner integration tests
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_correction_integration_triggers_assess_me():
    """User correction in initial messages triggers assess_me injection."""
    from unittest.mock import AsyncMock, patch
    ...
    assert result.had_injections, "Expected assess_me injection from correction"
    assess_count = sum(
        1 for m in result.messages
        if isinstance(m.get("content"), str) and "[assess]" in m["content"]
    )
    assert assess_count == 1, f"Expected exactly 1 assess_me, got {assess_count}"
    assert result.stop_reason == "completed"


@pytest.mark.asyncio
async def test_correction_guard_prevents_repeat():
    """_correction_handled guard prevents re-triggering on subsequent iterations."""
    from unittest.mock import AsyncMock, patch
    ...
    provider = MagicMock()
    call_count = 0
    assess_me_call_count = 0

    async def chat_fn(*, messages, **kwargs):
        nonlocal call_count
        call_count += 1
        # Keep making tool calls so we get multiple iterations
        if call_count <= 3:
            return LLMResponse(
                content=None,
                tool_calls=[ToolCallRequest(id=f"tc{call_count}", name="read_file", arguments={})],
    ...
    async def assess_me_spy(messages):
        nonlocal assess_me_call_count
        assess_me_call_count += 1
        return "请检查"

    with patch("nanobot.agent.runner._run_assess_me", assess_me_spy):
        result = await runner.run(AgentRunSpec(
            initial_messages=[
    ...
    # assess_me should be called exactly once despite correction message
    # still being in the window on subsequent iterations
    assert assess_me_call_count == 1, f"Expected 1 assess_me call, got {assess_me_call_count}"
    assert result.stop_reason == "completed"


@pytest.mark.asyncio
async def test_no_correction_normal_flow():
    """No correction patterns → no assess_me injection."""
    from nanobot.agent.runner import AgentRunner, AgentRunSpec
    ...
    runner = AgentRunner(provider)
    result = await runner.run(AgentRunSpec(
        initial_messages=[{"role": "user", "content": "你好，请读文件"}],
        tools=tools,
        model="test-model",
        max_iterations=10,
        max_tool_result_chars=_MAX_TOOL_RESULT_CHARS,
    ...
    assert not result.had_injections, "No injections expected without correction"
    assert result.stop_reason == "completed"
    assert result.final_content == "normal result"


# ---------------------------------------------------------------------------
# User correction — scenario tests
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_correction_then_tool_loop_both_fire():
    """Both tool loop recovery and user correction can fire assess_me in same iteration."""
    from unittest.mock import AsyncMock, patch
    ...
    class AlwaysFailTool(Tool):
        ...
        _tool_parameters_schema = {"type": "object", "properties": {}, "required": []}
    ...
    async def chat_fn(*, messages, **kwargs):
        ...
        return LLMResponse(
            content=None,
            tool_calls=[ToolCallRequest(id=f"tc{call_count}", name="fail_tool", arguments={})],
            usage={"prompt_tokens": 10, "completion_tokens": 5},
        )
    ...
    # Both signals present → at least one assess_me injected
    assess_msgs = [
        m for m in result.messages
        if isinstance(m.get("content"), str) and "[assess]" in m["content"]
    ]
    assert len(assess_msgs) >= 1, "Expected at least 1 assess_me injection"
    # Tool loop should eventually breaker if tool keeps failing
    assert result.stop_reason in ("tool_loop_breaker", "completed")


@pytest.mark.asyncio
async def test_correction_at_start_only_triggers_once():
    """User correction at conversation start triggers assess_me at most once."""
    from unittest.mock import AsyncMock, patch
    ...
    async def assess_spy(messages):
        nonlocal assess_count
        assess_count += 1
        return "请重新检查"

    with patch("nanobot.agent.runner._run_assess_me", assess_spy):
        result = await runner.run(AgentRunSpec(
            initial_messages=[
                {"role": "user", "content": "不对，你搞错了"},
                {"role": "user", "content": "读文件"},
            ],
            tools=tools,
            model="test-model",
    ...
    # Only one assess_me call despite correction persisting across iterations
    assert assess_count == 1, f"Expected 1 assess_me call, got {assess_count}"
    assert result.stop_reason == "completed"
