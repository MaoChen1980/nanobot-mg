"""Tests for SelfInsightHook (finding injection into agent context)."""
...
from nanobot.hooks.self_insight_hook import SelfInsightHook, _read_resolved_ids
...
class TestReadResolvedIds:
    def test_missing_file_returns_empty(self, tmp_path):
        ...
        with patch("nanobot.hooks.self_insight_hook.RESOLVED_FILE", path):
    ...
    def test_oserror_returns_empty(self, tmp_path):
        ...
        with patch("nanobot.hooks.self_insight_hook.RESOLVED_FILE", mock_file):
            assert _read_resolved_ids() == set()
...
class TestBuildFindingInsight:
    def test_missing_file_returns_none(self, hook):
    ...
    def test_json_error_returns_none(self, hook, tmp_path):
        ...
        with patch("nanobot.hooks.self_insight_hook.FINDINGS_FILE", findings_file):
            assert hook._build_finding_insight() is None
    ...
    def test_filters_resolved_ids(self, hook, tmp_path):
        ...
        with (
            patch("nanobot.hooks.self_insight_hook.FINDINGS_FILE", findings_file),
            patch("nanobot.hooks.self_insight_hook.RESOLVED_FILE", resolved_file),
        ):
...
class TestInjectInsight:
    def test_inserts_after_system_message(self):
        ...
        assert ctx.messages[1]["_source"] == "self_insight_hook"
        assert ctx.messages[1]["content"].startswith("[Self-Insight")
    ...
    def test_inserts_at_zero_when_no_system(self):
        ...
        assert ctx.messages[0]["_source"] == "self_insight_hook"

    def test_removes_stale_entries_first(self):
        hook = SelfInsightHook()
        ctx = FakeContext(messages=[
            {"role": "system", "content": "sys"},
            {"_source": "self_insight_hook", "content": "old"},
            {"role": "user", "content": "hi"},
        ])
        hook._inject_insight(ctx, "new insight")
        sources = [m.get("_source") for m in ctx.messages]
        assert sources.count("self_insight_hook") == 1
