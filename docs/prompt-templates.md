...
│   ├── evolution_agent.md                  # 自我进化 agent 的完整 prompt
...
| `agent/evolution_agent.md` | 自我进化子 agent 完整 prompt | `EvolutionAgent` 使用 |
...
#### 3.4.8 evolution_agent.md — 自我进化 agent

**用途**：根据对话回放（.pt 快照）发现并修复 NanoBot 自身的缺陷。

**修复范围**：
- **prompt 层**（`templates/agent/*.md`）：模糊用词、缺失指令、指令冲突、禁止模式缺失
- **code 层**（`nanobot/*.py`）：工具调用异常、context 缺失、循环控制问题
- **skill 层**（`workspace/skills/*/SKILL.md`）：触发条件问题、行为描述过时

**流程**：确认项目上下文 → 识别缺陷 → 判定是否修复 → 定点修改 → 验证 → 记录

#### 3.4.9 extractor_analysis.md — 记忆提取 prompt
...
session_parts.append(self._build_self_findings_section()) # 自我发现
session_parts.append(self._build_user_feedback_section()) # 用户反馈
...
### 6.3 通过 Self-Findings 注入自观察结论

`workspace/framework/self_findings.md` 由 `SelfDetectHook` 在每轮检测周期后写入，记录 AI 对自身工作模式的观察。此文件内容会被 `_build_self_findings_section()` 加载并注入到 system prompt 的 session parts 中。

### 6.4 通过 user_feedback.md 注入用户反馈模式

`workspace/framework/user_feedback.md` 由 `MemoryExtractor._process_user_feedback()` 写入，汇总用户纠正/拒绝信号的结构化模式。此内容同样被注入到 session parts 中。

### 6.5 创建自定义 Agent 提示词

模板目录中的多个完整 prompt（如 `assess_me.md`、`evaluator.md`、`evolution_agent.md`、`extractor_*.md`）都是独立子 agent 的完整 prompt。你可以基于这些模板创建新的子 agent prompt：
...
│   └─ events/（最近7天事件）
│
├─ _build_self_findings_section()            → 自我发现注入
│
├─ _build_user_feedback_section()            → 用户反馈注入
│
