...
- **Team Communication Tools** — NotifyOrchestratorTool, SendMessageTool
...
通过 SendMessageTool 实现。该工具持有 SubagentManager 引用：

```python
tools.register(SendMessageTool(manager=self, subagent_id=task_id, subagent_label=label))
```

子代理在提示中可以看到其他活跃子代理的列表（通过 `list_running_statuses()`），并可以向它们发送消息：

- 发送方使用 `SendMessageTool` 写入目标子代理的收件箱
