...
export function SettingsView({ onBackToChat, onModelNameChange }: SettingsViewProps) {
  const [settings, setSettings] = useState<SettingsPayload | null>(null);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [form, setForm] = useState({ model: "", provider: "auto" });

  const applyPayload = useCallback((payload: SettingsPayload) => {
    setSettings(payload);
    setForm({ model: payload.agent.model, provider: payload.agent.provider });
  }, []);
  ...
  const dirty = useMemo(() => {
    ...
    return form.model !== settings.agent.model || form.provider !== settings.agent.provider;
  }, [form, settings]);
    ...
    <div className="min-h-0 flex-1 overflow-y-auto bg-background">
      <main className="mx-auto w-full max-w-[1000px] px-6 py-6">
        ...
        {loading ? (
          ...
          <SettingsGroup><SettingsRow title="Could not load settings">
            <span className="max-w-[520px] text-sm text-muted-foreground">{error}</span>
          </SettingsRow></SettingsGroup>
        ) : settings ? (
          <SettingsSection form={form} setForm={setForm} settings={settings} dirty={dirty} saving={saving} onSave={save} />
        ) : null}
      </main>
    </div>
  );
...
function SettingsSection({ form, setForm, settings, dirty, saving, onSave }: {
  form: { model: string; provider: string };
  setForm: React.Dispatch<React.SetStateAction<{ model: string; provider: string }>>;
  settings: SettingsPayload; dirty: boolean; saving: boolean; onSave: () => void;
}) {
  return (
    <div className="space-y-7">
        ...
        <SettingsGroup>
          <SettingsRow title="Provider">
            <select value={form.provider}
              ...
              className={cn("h-8 w-[210px] rounded-md border border-input bg-background px-2 text-sm",
                "outline-none transition-colors hover:bg-accent focus-visible:ring-2 focus-visible:ring-ring")}>
              {settings.providers.map((p) => <option key={p.name} value={p.name}>{p.label}</option>)}
            </select>
          </SettingsRow>
      ...
      <section>
        ...
        <SettingsGroup>
          <SettingsRow title="Language"><LanguageSwitcher /></SettingsRow>
        </SettingsGroup>
      </section>
    </div>
  );
...
function SettingsGroup({ children }: { children: React.ReactNode }) {
  return <div className="overflow-hidden rounded-xl border border-border/60 bg-card/80">
