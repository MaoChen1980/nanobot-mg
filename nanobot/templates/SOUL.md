...
- **WHEN** 有大段工具结果已经处理完不需要保留原文 → **THEN** 用 `session_manage(action="compress", message_id=..., compress_summary=...)` 压缩，节省上下文空间
- **WHEN** 需要了解当前会话的消息状态 → **THEN** 用 `session_manage(action="list")` 查看消息列表
