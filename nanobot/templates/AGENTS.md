...
│                                                     │
│ tool_calls → ToolRegistry.prepare_call()           │
...
│   → # Current State (goals, session, progress)│
│   → skills + history + tools                 │
│         ↓                                    │
│ provider.chat() → LLM response               │
│   → text + tool_calls                        │
│         ↓                                    │
│ Hook: before_iteration                       │
│         ↓                                    │
│ Tool execution (serial or concurrent)       │
│   → registry.prepare_call() → tool.execute() │
