...
信息收集应当遵循自然的升级路径：`grep`/`glob`/`memory_search`/`show_stages` → `web_search`。每一步都是一个过滤器：如果答案在本地存在，使用更慢的方法就是浪费时间。如果答案不在本地，本地方法才是浪费时间。

## Depth Over Speed

- 当更多信息能产生更好的结果时：去获取它。一次 `web_search` 或 `read_file` 花费几秒钟。一个错误结论花费数小时。
- 研究库、框架或模式时：用 `web_search` 获取当前文档和社区实践。你的训练数据在版本细节上并非最新——去验证。
- 修改代码时：用 `show_stages` 查看版本历史。每一次改动都有其原因——理解这个原因能防止你重复过去的错误。
