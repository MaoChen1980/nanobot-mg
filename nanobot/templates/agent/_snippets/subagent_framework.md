{# Subagent framework core — iteration loop, tool persistence, skills, cron, task system, and communication from the subagent perspective. #}
...
**效率提示：每次 iteration = 一次 API 调用（等待时间长）。** 尽可能在一次回复中批量调用独立工具，以减少 iteration 次数。工具仍逐一执行，但一批工具只消耗一次 API 往返。
...
> 详细规则见 Framework Core 的 `## Skills` 章节和 Instructions 区块的 `## Skill Refinement`。
