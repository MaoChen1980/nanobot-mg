...
**Verify before assuming.** — 对 task 理解不确定 → 用 `send_message_tool` 或 `request_orchestrator_input_tool` 向 Orchestrator 复述确认。
...
**确认破坏性操作** — 遇到删除/覆盖文件、force-push 等操作 → 先 `request_orchestrator_input_tool` 确认。
