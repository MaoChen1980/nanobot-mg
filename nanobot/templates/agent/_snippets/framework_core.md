...
`tasks/tree.json` 是系统的**永久项目索引**，它会在指令系统以可视化树图注入。
completed 节点永远保留，LLM 通过注入始终知道"以前做过什么项目"，随时读取项目档案。

**文件说明：**
| 文件 | 用途 | 生命周期 |
|------|------|---------|
| `{{ workspace_path }}/tasks/tree.json` | 永久项目索引（JSON）。所有项目全量记录，schema 见 `tree.schema.md` | 持久，completed 节点永远保留 |
| `{{ workspace_path }}/tasks/CURRENT.md` | **当前项目**进度和状态（纵向：做到哪了、下一步） | 随项目节点生灭，归档后清空 |
| `{{ workspace_path }}/tasks/team_board.md` | **当前项目**事实发现（横向：洞察/踩坑/发现/状态变化） | 随项目节点生灭，归档后清空 |
...
**关键规则：**
- 不删除历史节点。failed/paused/completed 保留轨迹，tree.json 是永久索引
- 子节点 completed ≠ 父节点完成——验证 criteria
- CURRENT.md 和 team_board.md 只跟踪**当前正在进行的项目节点**，归档后清空
...
user: "Subagent 返回了结果。\n\n[Subagent 内容]\n\n记住原始任务目标，所有决策围绕最终交付。\n\n请检查 Subagent 状态轮数、处理/更新最新任务状态。翻阅 `{{ workspace_path }}/tasks/team_board.md` 看 Subagent 有没有写出值得关注的事实/踩坑/洞察。"
```
