...
**⚠️ skill 加载必须基于显式指令，禁止无提示预加载：**

- 当 reminder/cron 消息**明确要求**加载 skill（如「先用 skill_search 加载 X skill」「请加载 skill X」）→ 执行 `skill_search` → `read_file` SKILL.md
- 当 reminder/cron 消息**只要求执行任务**（如「执行脚本 + 发送报告」「推送 MGA 分析」）→ **禁止**主动执行 `skill_search` 或 `read_file` SKILL.md
- skill 是任务导向的工具，不是通用背景知识，不允许「预加载」作为背景储备

**典型违规：**
```
❌ cron reminder: "执行脚本 + 发送报告"（无 skill 加载指令）
   tool_calls: [exec → message → skill_search(market-game-analysis) → read_file(SKILL.md)]
   → 违规：没有 skill 加载指令就加载了 skill，market-game-analysis 是执行目标而非加载目标

✅ 合规：
   cron reminder: "执行脚本 + 发送报告"
   tool_calls: [exec → message]

   cron reminder: "先用 skill_search 加载 X skill，然后执行分析"
   tool_calls: [skill_search → read_file(SKILL.md) → exec → message]
```
...
assess_me findings + 压制指令收敛后，以下输出均为违规：
- ❌ 「tool_calls 执行完毕，content = ""」
- ❌ 「(Zero text per assess suppress.)」
- ❌ 「assess findings 已记录」「修复已落地」「验证通过」
- ❌ 任何 ✅清单、状态摘要、进度汇报
- ❌ 任何提及「压制」「收敛」「skipped」的文字

> **关键认知：** exec 返回 `exit 0` = 执行成功证明；message() 返回 `"status": "ok"` = 发送成功证明。这些 tool 结果本身就是成功的证明，无需用文字再次确认。assistant content = `""` 是框架级约束，不是 agent 可以选择的行为。
