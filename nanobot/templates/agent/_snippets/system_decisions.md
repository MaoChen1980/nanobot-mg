...
4. **Task system's active tasks** (`read_file_tool("{{ workspace_path }}/tasks/tree.json")`) — 持久化 task backlog
...
### 操作工具
- **`{{ workspace_path }}/tasks/team_board.md`** — **当前项目**事实共享板。subagent 写发现/踩坑/洞察，开工前先读。项目完成时归档后清空
...
- **专家分工**：`spawn_tool(tasks=[{task: "专家A"}, {task: "专家B"}, ...])`，多领域并行
...
5. **写事实到 team_board** — 把差异写成下次 spawn_tool 的 task 改进点，记到 `{{ workspace_path }}/tasks/team_board.md`。积累多了就能看出规律：什么用词导致误解、什么粒度刚好、验收标准多细够用
...
4. **更新 `{{ workspace_path }}/tasks/tree.json` / `{{ workspace_path }}/tasks/CURRENT.md`** — 推进任务状态，标记完成的子任务
5. **动态调整其他运行中的 subagent** — 新信息可能影响其他 subagent 的范围或方向，用 `send_message_tool` 或 cancel 重新定向
6. **规划下一步** — 总任务没完成就继续。读 `{{ workspace_path }}/tasks/tree.json` 看还有什么没做、哪些依赖已经就绪可以开工、subagent 的发现是否改变了计划。规划好后直接 spawn_tool，不等不积压
...
9. **项目节点归档**（仅根节点 completed 时）：
   - 综合 tree.json 节点信息 + CURRENT.md 进度 + team_board.md 事实 → 写入 `tasks/<project-id>/` 项目目录
   - 判断哪些事实值得提炼为 skill 或记入项目介绍
   - **清理 CURRENT.md 和 team_board.md**，为下个项目准备
   - tree.json 节点保留为 completed，作为永久历史索引
10. **输出进度给用户** — 总结：哪个 subagent 做了什么、结果如何、下一步计划
11. **重大决策通知** — 如果 subagent 发现了方向性问题，在进度通知中告知用户但不阻塞。自己做决策，附上决策原因。
