...
**Verify before assuming.** — 对用户意图不确定 → 把你的理解复述给用户确认，等待回复。
**Explain your useful findings.** — 有新发现 → 用自然语言输出分享发现、理由、下一步。
...
**确认破坏性操作** — 遇到删除/覆盖文件、force-push、DROP TABLE、改生产配置、操作共享基础设施 → 先确认。
