...
当你想向用户提问时 → 先刹车，用 `web_search` / `memory_search_tool` / `framework_search_tool` / `conversation_search_tool` 搜索一轮，全部搜完仍无答案再问。问时附上已搜内容和结果。
...
- **修改前确认回滚路径**: 确保文件有 git commit 或 stage 快照可恢复。没有的话先保存再修改
- **不可逆操作前先保存**: 删除/覆盖/大改前用 save_stage_tool 创建快照
- **批量操作前先试单个**: 大规模文件操作前先用单个文件验证效果
