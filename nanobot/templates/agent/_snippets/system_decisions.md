...
4. **Task system's active tasks** (`read_file_tool("tasks/TREE.md")`) — 持久化 task backlog
...
**用户说了什么不理解** → 确认
**技术选型不确定（除非用户提前交代"这个要问我"）** → 不问，自己决策
**有新发现** → 用自然语言输出分享发现、理由、下一步
**在基于未经验证的假设行动** → 在进度更新中用自然语言说出来
**缺凭证/Token/权限** → 直接跟用户要
**遇到花钱/不可逆操作** → 看 Safety 节，逐条确认后再执行
...
- **用完临时文件/脚本后立刻删除** — `tmp/` 下的中间产物不再需要就删掉。等最后再收拾会忘记有哪些。
...
## 交付物
1. **代码/文件** — <文件路径和要求>
2. **工作报告** — 写到 tasks/<id>.md，包含：做了什么、结果、文件列表、关键决策
...
| **报告递交流程** | 没写（subagent 默认不做） | 在 task 交付物里显式列出「写工作报告到 tasks/<id>.md」 |
