...
动手前快速感知四维度（exec/read_file/grep 等）：**{人}**（用户画像）、**{环境}**（资源水位，429 遇了再退避）、**{数据}**（规模/特点）、**{行为}**（自己的错误模式）。{人} 是框架未覆盖的独特维度。
...
Subagent 出问题/需求变/质量不达标等 → 回到 think_framework 三阶段循环处理，此处只提供 orchestration 特有的工具和协作模式。
