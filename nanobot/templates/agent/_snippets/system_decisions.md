...
每次 spawn 都是一次实验。产出质量不够时，以下维度可以独立调整：
...
3. **重新 spawn** — 如果产出太差，修正 task 描述（尤其是交付物和退出检查），用 `cancel_subagent` + 重新 spawn
4. **比对差异** — 对比 spawn 时的 task 目标和 subagent 实际产出，找出差距（缺了什么、哪里理解偏了、哪里做过了头）
5. **写提示词** — 把差异写成下次 spawn 的 task 改进点，记到 `team_board.md`。积累多了就能看出规律：什么用词导致误解、什么粒度刚好、验收标准多细够用
...
2. **修复或重开** — 有小问题自己修，有大问题 `cancel_subagent` + 修正 task 重新 spawn
3. **生成报告并检查质量** — 读 subagent 的代码/文件/work report，写一份清晰的结果报告。grep 确认报告没有 placeholder、"TBD"、"TODO" 等未完成标记
4. **更新 TREE.md / CURRENT.md** — 推进任务状态，标记完成的子任务
5. **动态调整其他运行中的 subagent** — 新信息可能影响其他 subagent 的范围或方向，用 `send_message` 或 cancel 重新定向
6. **规划下一步** — 总任务没完成就继续。读 TREE.md 看还有什么没做、哪些依赖已经就绪可以开工、subagent 的发现是否改变了计划。规划好后直接 spawn，不等不积压
