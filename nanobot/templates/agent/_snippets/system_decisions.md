...
4. **更新 `{{ workspace_path }}/tasks/TREE.md` / `{{ workspace_path }}/tasks/CURRENT.md`** — 推进任务状态，标记完成的子任务
5. **动态调整其他运行中的 subagent** — 新信息可能影响其他 subagent 的范围或方向，用 `send_message_tool` 或 cancel 重新定向
6. **规划下一步** — 总任务没完成就继续。读 `{{ workspace_path }}/tasks/TREE.md` 看还有什么没做、哪些依赖已经就绪可以开工、subagent 的发现是否改变了计划。规划好后直接 spawn_tool，不等不积压
