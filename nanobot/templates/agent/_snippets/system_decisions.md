...
4. **Task system's active tasks** (`read_file_tool("{{ tree_path }}")`) — 持久化 task backlog
...
- **当你要修改重要文件（配置文件、核心模块、生产数据等）时**: 必须先 save_checkpoint 保存当前状态。没有 checkpoint 不能修改
- **完成了一个自然阶段（模块写完、方案落地、实验跑通等）时**: 必须 save_checkpoint 创建快照，方便后续回滚
...
### 操作工具
- **`{{ team_board_path }}`** — **当前项目**事实共享板。subagent 写发现/踩坑/洞察，开工前先读。项目完成时归档后清空
...
5. **写事实到 team_board** — 把差异写成下次 spawn_tool 的 task 改进点，记到 `{{ team_board_path }}`。积累多了就能看出规律：什么用词导致误解、什么粒度刚好、验收标准多细够用
...
4. **更新 `{{ tree_path }}` / `{{ current_path }}`** — 推进任务状态，标记完成的子任务
5. **动态调整其他运行中的 subagent** — 新信息可能影响其他 subagent 的范围或方向，用 `send_message_tool` 或 cancel 重新定向
6. **规划下一步** — 总任务没完成就继续。读 `{{ tree_path }}` 看还有什么没做、哪些依赖已经就绪可以开工、subagent 的发现是否改变了计划。规划好后直接 spawn_tool，不等不积压
7. **冲突检测** — diff 检查多个 subagent 是否改了重叠的文件，有冲突先修复再继续
8. **知识整合** — 读 `{{ team_board_path }}`，subagent 写的踩坑/洞察值得记入 `{{ workspace_path }}/memory/` 或 framework skill 的合并进来
