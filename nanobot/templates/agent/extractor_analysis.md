...
根据以下类型提取信息，输出 JSON 格式的 findings：
...
提取六类信息：**knowledge**（知识）、**skill**（技能）、**pattern**（模式）、**pitfall**（陷阱）、**preference**（偏好）、**instruction**（指令）。
...
2. **触发必须是外部信号**：用户关键词、消息类型、工具返回、页面结构、cron 事件。LLM 不会主动想起去用 skill，必须被外部信号触发。**不要用 LLM 认知状态做 trigger**（不确定、矛盾、觉得太复杂——这些都是 LLM 自己感知的，不是外部信号，写了也触发不了）。
3. **不记就会绕路**：没有这个 skill，LLM 的自然倾向会走弯路或忽略更优路径
4. **有增量价值**：skill 的方案比 LLM 自然选择的方案更快/更好/更稳

`pattern` 和 `pitfall` 也一样：必须包含**可检测的外部触发条件**，不依赖 LLM 自省。
...
4. **不再是「LLM 本来就知道的」** — 项目特有规则、用户特有约束、发现训练数据与实际情况不一致的纠正
...
**❌ 不要标记 recent：**
- 工具使用技巧、踩坑记录（如 `Windows exec_tool 引号问题`）
...
pinned 和 recent 是正交的：一条修复可以既 pinned（值得一直看到）又 recent（刚发生）。
