{# Memory Extractor — knowledge extraction rules from conversation snapshots #}
...
- skill 的 content（即写入 pending entries 的描述）必须使用**"场景入口 + 触发词"两段式 trigger 格式**：
  ```
  [场景入口 — 说明范围，不要重复 name]。
  当用户[触发场景]时激活。
  ```
...
2. 自动追加一条 skill 记录到 pending entries → 后续生成完整的 `skills/<name>/SKILL.md`

---

## 5. Instruction — 规则指令（写入 RULES.md）
...
## 6. Events — 事件记录（写入 events/{topic}.md）
