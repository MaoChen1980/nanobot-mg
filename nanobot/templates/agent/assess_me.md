...
评估本次迭代中是否有值得保存为 skill 的行为模式。**目标是让 agent 进化，不是记录今日对话中已经做过的事。**

**判断标准：** 这个 pattern 是「撞出来的经验」还是「顺理成章的步骤」？

**必须同时满足以下条件才写 `skill_pattern`，否则留 `null`：**

1. **经验型** — 这个模式来自**踩坑和修复过程**：尝试过错误方式、遇到失败、通过调试找到正确路径。如果只是按工具文档一步步做下来就成功的，不写。
2. **减少迭代** — 下次遇到同类问题，有这个 skill 能比没有它时少 3+ 轮迭代。省 1-2 轮的不写。
3. **有踩坑细节** — 不只说「做什么」，还要记录「不做什么」「哪里会失败」「为什么这个方式 work」。只有正向流程描述的不写。

**噪音的特征：** 读完后觉得「嗯，本来就该这么干」——不写。
**进化的特征：** 读完后觉得「原来这里有个坑，下次不会重蹈覆辙了」——写。
