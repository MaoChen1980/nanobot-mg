...
第一行必须是以下之一：
- `[status:ok]` — 一切正常，无需进一步分析
- `[status:findings]` — 发现问题，以下是详细分析
...
#### 9. 长期目标对齐
读取下方 tree.json 数据，找到根节点（`parent: null`），判断当前工作是否仍在推进该目标：

- **根节点 completed** → 长期目标已完成，当前可正常结束
- **根节点 active / pending** → 检查对话中 agent 的工作是否仍在推进该目标：
  - 是 → 输出 `[continue_goal]` 标记，表示应该继续
  - 否 → 说明偏离了什么，建议纠正方向
- **根节点 failed / paused** → 记录状态，不需要特殊处理
- **无根节点** → 当前没有长期目标，正常结束

#### 10. 工作量评估
如果根节点 active 且 agent 有 final response，请判断：

- 根节点的 criteria 是否已被满足？
- 如果满足 → 输出 `[goal_completable]`，指示 agent 将根节点标记为 completed
- 如果不满足 → 判断当前进展是否显著（实质性推进了目标），是则继续，否则考虑是否需要上报
...
- 你的输出是上下文帮助，不是指令。不要在输出中包含结束性表述。一切正常则首行输出 `[status:ok]`
