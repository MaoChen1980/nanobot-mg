...
评估本次迭代中是否有值得保存为 skill 的行为模式。**目标是让 agent 进化，不是记录 agent 本来就会的事。**

**必须同时满足以下条件才写 `skill_pattern`，否则留 `null`：**

1. **非推理可得** — 这个模式 LLM 无法通过推理推导出来（如：特定平台的兼容性坑、某个工具的非预期行为、环境特性）。LLM 靠常识就能想出来的标准做法**不写**。
2. **减少迭代** — 下次遇到同类问题，有这个 skill 能比从头推理少 3+ 轮迭代。只省 1-2 轮的不写。
3. **有踩坑细节** — 不只说「做什么」，还要记录「不做什么」「哪里会失败」「为什么这个方式 work」。纯流程描述不写。

**反例（噪音，不写）：**
- "先读需求文档再写代码" — LLM 本来就会
- "使用 adb shell input tap 点击坐标" — 这是工具用法，不是 skill
- "测试失败时先看日志" — 标准调试流程

**正例（进化，写）：**
- "Windows 上 adb shell 内嵌管道符号会被 cmd.exe 截断，需要用 Python subprocess 分步执行" — LLM 靠推理撞不出来
- "某 adb 命令在模拟器上返回空但在真机上正常，因为模拟器缺驱动层" — 环境差异，省 5 轮排查

**高质量 skill 的特征：** 下次迭代读了这个 skill，能比没有它时少走 3 轮以上弯路。
