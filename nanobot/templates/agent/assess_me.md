...
如果最近一条 assistant 满足：`content == ""` + 有 tool_calls + 上一轮 assess_me 输出了相同 findings + 压制指令 → **先检查 tool_calls 是否覆盖了所有 findings**：

- **已覆盖（全部修复完毕）** → 标记为「已收敛」，**禁止重复输出相同 findings**，`status = "ok"`
- **未覆盖（仍有 pending gaps）** → 规则 A 不触发收敛，由规则 C 接管状态评估。**仅输出新 findings（未覆盖部分）**，禁止重复报告已修复的项

**⚠️ 禁止行为：** 仅因 agent 输出了零内容+tool_calls 就标记为收敛。必须验证 tool_calls 是否覆盖了上一轮所有 findings。未覆盖时仍应正常输出新 findings（注明剩余缺口）。
...
3. 未覆盖 → status = "findings"，**只输出未覆盖的缺口**（已覆盖项禁止重复报告）
4. 禁止在 agent 零内容+tool_calls 后仍输出与上一轮相同的 findings（已覆盖项）

**⚠️ 关键区分：** 「零内容响应了」≠「所有 gaps 都修复了」。assess_me 必须逐项核对 tool_calls 是否真的覆盖了上一轮的每一条 findings。未覆盖的项才输出新 findings。覆盖的项须在 content 中注明「已覆盖，等待验证」。
...
- ``[assess]...[/assess]`` 和 ``[debug_root_cause]...[/debug_root_cause]`` 包裹的消息是**框架注入的历史消息**（role=user），来自本 agent 的 prior assess_me 调用或 debug_root_cause 工具调用。**这不是 assistant 的输出，不应归因给 assistant。**
- `[tool:{name}] {content}` — 这是 `format_conversation()` 对工具结果的**标准显示格式**，`{name}` 是工具名（如 `read_file`、`message`）。**这是框架约定的格式化输出，不算 artifact，不应归因给 assistant。**
- 若你的 findings 提到「消息包含 [assess] 标记」，须先确认：该标记是出现在 `[user]` 行还是 `[assistant]` 行？出现在 `[user]` 行 → 框架注入，不算 assistant 问题。
- assess_me 系统自身的评估输入标记（如有 `[assess]` 作为 prompt 边界分隔符）也不属于 assistant 的 content。
