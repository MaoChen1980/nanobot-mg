...
- **高风险操作前必查坑记录 → 执行 git pull --rebase / git merge / git rebase 等有潜在冲突风险的操作前，先用 `grep` 搜索 `memory/working.md` 中是否有相关踩坑记录（关键词：git、rebase、conflict、GIT_EDITOR）。有记录则先阅读确认预防措施再执行。禁止跳过此步骤直接执行**
- edit_file 报 old_text not found → **必须用 grep 搜索 old_text 关键词片段定位实际内容**，再 read_file 该区域确认实际内容，用实际内容重试。禁止仅凭之前读到的行号/内容直接 retry
...
- 工具不可用 → 换方案或告知用户，不硬撑
- **被指令加载的 skill 文件不存在（read_file 返回 File not found）→ 立即用 `message()` 通知用户该 skill 不可用，记录该 skill 为 skipped 状态，改用 grep/read_file 替代方案继续推进，不重试同一路径**
...
**Subagent 并行分析结果合并 — 统一摘要强制规则:**
TRIGGER: 多个并行 subagent 各自产出独立的分析/报告文件（如 gap_*.md、audit_*.md、analysis_*.md）
ACTION: 主 agent 必须执行以下三步，不得跳过：
1. **读取所有产出文件** — 用 glob 列出所有报告文件，逐个 read_file 读取内容
2. **合并为统一摘要** — 按模块/维度/主题分类整合，提取关键结论、优先级排序、待处理项
3. **呈现清晰结论** — 给用户一个结构化的总览（如按 P0/P1/P2 分组），而不是列举文件路径让用户自行拼凑
**禁止行为：**
- ❌ 只列举文件路径（如"gap 分析已完成，报告在 tmp/gap_*.md"）而不读取合并
- ❌ 说"正在等待结果"但文件已存在（文件落地 = 分析完成，必须立即读取）
- ❌ 不验证文件内容完整性就假设结果正确
...
assess_me 报告描述的是"agent 应该做了什么但没做"。**assess_me skill 加载核心规则见 `framework_core.md` 顶部 TL;DR 框。**

#### 1. Skill 加载 — assess_me 强制要求（见 framework_core.md TL;DR）

**核心规则：** assess_me skill 加载指令 = **本轮第一件事**，见 framework_core.md 顶部。

**强制检查点（每次 iteration 开始时必查）：**
- [ ] assess_me 是否提及「skill 未被使用」或「请加载 skill X」？
- [ ] assess_me 是否包含「立即加载」类显式指令？

**如果任一为 YES → 立即执行：** `skill_search` → `read_file` SKILL.md → 按 Steps 执行 → 才能做其他工作

**典型违规（见 framework_core.md）：** assess 要求 skill，agent 先 git push/grep/working.md → 违反强制优先级。
...
#### 7. 评估结果合并验证 — subagent 自纠后必须二次验证

**TRIGGER:** assess subagent 报告出现误报并自纠（如将「已实装」修正为「NOT_NEEDED」或将「COMPLETE」修正为「PARTIAL」），agent 准备合并修正结果到 team board 之前

**问题背景：** assess 方法论无法可靠区分「已实现」「死代码」「从未引用」三种状态。subagent 自纠后的结论仍可能存在错误（如将「PARTIAL」误报为「NOT_NEEDED」），直接信任修正会导致错误累积，team board 状态将不可信。

**强制验证步骤：**

1. **提取关键声明** — 从 assess 报告中识别所有状态标记词：「COMPLETE」「已实装」「PARTIAL」「NOT_NEEDED」「NOT_FOUND」「未实现」
2. **grep 交叉验证** — 对每个关键声明，用 `grep` 在目标源码中验证：
   - 「COMPLETE」/「已实装」→ grep 确认函数/接口定义存在且有调用点
   - 「PARTIAL」→ grep 确认存在部分实现，识别缺失部分
   - 「NOT_NEEDED」/「NOT_FOUND」→ grep 确认无相关代码或路径
3. **不一致处理** — 如果 grep 结果与 assess 报告不符，将差异记录到 team board 备注栏，标记为「待人工确认」
4. **验证后才合并** — 完成上述步骤后才能将评估结果合入 team board

**禁止行为：**
- ❌ subagent 自纠后直接合并，不做二次验证
- ❌ 假设 subagent 的修正必然正确
- ❌ 只验证一处声明而忽略其他状态标记词

#### 8. 诊断与任务并行化 — assess_me 多步计划不应阻塞核心任务

**TRIGGER: assess_me 提供多步 debug_root_cause 计划（如 6 步诊断方案），且同时有 P0 核心任务待执行**

**问题背景：** assess_me 提供的诊断计划是辅助性建议，不应成为阻塞核心任务的先决条件。当诊断步骤可与任务执行并行时，应优先推进任务进度。

**判断标准：**
| 场景 | 行动 |
|------|------|
| 诊断是任务的前置条件（如：环境未就绪无法构建） | 先完成诊断，确保任务可执行 |
| 诊断与任务可并行（如：JAVA_HOME 诊断 + MessageInjector 集成） | 并行执行，诊断背景推进，任务优先交付 |
| 诊断阻塞核心任务但本身耗时 | 将诊断拆分：快速验证项 → 执行任务 → 后续诊断细节 |

**强制行为：**
1. **任务进度优先** — assess_me 诊断计划是背景任务，核心 P0/P1 任务应持续推进
2. **快速根因验证** — 能快速确认的诊断项（如单次 grep/read_file）立即做，不攒成一堆
3. **并行化判断** — 每轮迭代前判断：诊断和任务是否独立？独立则并行执行
4. **阶段性交付** — 核心任务有阶段性成果就交付，不等诊断完成
...
**典型模式：**
```
问题：assess_me 提供 6 步诊断计划，agent 完整执行后才修复 + 提交

正确行为：
- iteration 1: 启动诊断（grep A）+ 启动核心任务（read_file X）→ 并行
- iteration 2: 诊断结果返回 + 任务进展更新 → 继续任务
- iteration 3: 诊断发现根因 + 任务已完成 → 合并提交
```
