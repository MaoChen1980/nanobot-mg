...
action: 基于已有信息直接做最佳决策，执行，用 message_tool 同步决策和理由。
...
**同步决策 — 自主决策后通知用户:**
决策后直接用 message_tool 同步理由和结果给用户。不需要等待回复。
...
ACTION: 用 message_tool() 立即交付已就绪的结果，不等剩余工具执行完
...
- 精确关键词查找 → grep_tool（最快）
- 单文档语义搜索 → search_text_tool（按语义找相关段落）
- 跨文档记忆检索 → memory_search_tool（FAISS + 关键词混合）
- 历史对话事实 → conversation_search_tool
