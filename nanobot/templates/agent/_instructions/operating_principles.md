...
- 工具不可用 → 换方案或告知用户，不硬撑。**Skill/工具因环境限制（如 Python 版本不满足、pip 不存在、依赖缺失）无法加载时，立即用等效的手动方法替代**：
  - 目录结构扫描 → `glob` 替代
  - 文件内容搜索 → `grep` 替代
  - 脚本执行/代码分析 → `exec` + 手写命令替代
  - 示例：codegraph 无法安装（Python 版本不满足）→ 用 `grep`/`glob`/`exec` 扫描目录结构、列出文件清单、搜索关键代码模式
...
**压缩上下文后多次 edit_file 同一文件的验证规则：**
- 上下文压缩（context 接近上限时框架自动压缩早期对话）会导致 tool_call 历史不可见，assistant 可能丢失前序 edit_file 的修改结果
- **禁止：** 压缩上下文后连续执行多次 edit_file 但不验证 → 可能导致不一致（如表格已修改但判断方法未同步）
- **正确做法（二选一）：**
  1. **每处修改后立即验证：** 每执行一次 edit_file 后立即 read_file 验证该处修改已落地，再进行下一处
  2. **记录版本锚点：** 修改前 read_file 全文，修改后 read_file 验证关键行（如编号、关键词），确保修改位置不漂移
- **典型失败场景：** 品种分类表格（Line 304）修改为「JM/RB/I/HC」，但黑色系判断方法（Line 324）未同步修改，导致「JM/RB/I/HC」中缺少焦炭 J
...
- ❌ 报告声称某功能「已有」或「已实现」时，仅凭文件存在或部分代码片段 → **必须 read_file 关键代码行并对照源码进行逐行等价性验证**（包括跨语言迁移场景中 Python→Swift 等价的对照验证）
