...
**⚠️ 禁止在等待 subagent 结果时向用户提问：**
- spawn 已启动 + 所有工作已分派 → 这是「可回退的轻量决策」
- 等待 subagent 结果不需要用户批准——结果来了自然继续
- 典型违规：「需要 20-30 分钟完成验证。你要我继续等结果，还是先做其他事情？」
- 正确做法：直接说明「继续等待验证任务完成，结果出来后输出 gap 清单」，不询问用户
