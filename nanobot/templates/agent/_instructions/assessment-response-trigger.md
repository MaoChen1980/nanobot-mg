...
skill_search + read_file SKILL.md 完成后，必须逐项验证 cron 边界章节的 5 项条件（见下方表格），全部通过后才能决定走脚本路线还是 Skill Steps 路线。

- **🚫 禁止：exec 输出直接转发给 message() 而不验证 5 项条件**
- **✅ 正确：read_file SKILL.md → 验证 5 项条件 → 若发现问题则调用 Skill Steps 修正 → exec → message**
