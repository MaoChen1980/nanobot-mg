...
STEP A: 识别到 Rule 3 TRIGGER → 立即执行 skill 加载链
  ...
  → ⚠️ **优先级说明：assess 压制收敛期内 reminder 含 skill 指令时，skill 加载链优先级最高，禁止被 reminder 的 exec+message 打断**
...
- ⚠️ 典型错误 2：read_file 时使用 `offset/limit` 只读取部分行（如 lines 1-5/258），未读取 SKILL.md 全文（258 行）→ **CRON 边界 6 项条件第 5 项「agent 已执行 skill_search + read_file」要求完整加载，当前仅部分加载 = 不满足条件 5**
...
**⚠️⚠️⚠️ CRON 边界 6 项条件验证（强制执行，禁止跳过）⚠️⚠️⚠️**

skill_search + read_file SKILL.md 完成后，**必须逐项验证 cron 边界章节的 6 项条件**，全部通过后才能决定走脚本路线还是 Skill Steps 路线：

| 条件 | 验证方法 | 若不满足 |
|------|---------|---------|
| **0. 脚本文件存在** | `glob`**在 `workspace/tmp/` 目录搜索 `mga_full_analysis.py`**，确认文件存在 | **立即用 `exec python tmp/mga_full_analysis.py` 验证脚本可用性**；若脚本不存在于预期路径，搜索实际位置：`grep -r "def main\|if __name__" tmp/*.py` |
...
| **5. agent 已执行 skill_search + read_file SKILL.md 全文** | **检查 tool_calls 历史中 read_file 是否完整读取（当前文件行数 ≤258 行，完整加载 ≠ 仅读前 N 行）** | **必须执行完整 Skill Steps（不能走脚本路线）** |
...
**⚠️ 5 项条件验证（CRON 边界强制执行）：**

`read_file` SKILL.md 完成后，**必须逐项验证 cron 边界的 5 项条件**，全部通过后才能决定走脚本路线还是 Skill Steps 路线：

| 条件 | 验证方法 | 若不满足 |
|------|---------|---------|
| 1. cron reminder 明确要求加载 skill | payload 含「market-game-analysis」或「MGA」 | 不走 skill 加载链 |
| 2. agent 尚未执行 skill_search | 当前轮次无 skill_search 调用 | 必须执行 skill_search |
| 3. agent 尚未 read_file SKILL.md | 当前轮次无 read_file 调用 | 必须 read_file 全文 |
| 4. SKILL.md 路径有效 | skill_search 返回路径 | 用 skill_search 重新定位 |
| 5. cron 未进入压制期 | assess_me 无「findings + 压制指令」共存 | 按压制协议执行 |

**🚫 禁止绕过 5 项条件验证：**
- 禁止：exec 输出直接转发给 message() 而不验证条件
- 禁止：用「数据已由脚本获取」「exec 输出已包含分析结果」等理由跳过验证
- **正确：skill_search → read_file SKILL.md 全文 → 验证 5 项条件 → 根据结果走对应路线**
...
|---|---|
| 「skill 加载不完整（仅前 N 行）」+ **「Steps 未执行」** + **内容不足以执行任何 Step** | **先完成 SKILL.md 加载**：继续分片 read_file 直至全文加载（258 行）<br>→ 全文加载完成后 → **立即执行 Steps**<br>→ **禁止：内容不足时跳过 Steps** |
...
- **第一判断：内容是否足以执行任何 Step**
  - SKILL.md 仅读取前 5 行（lines 1-5 of 258）→ 内容不足以执行任何 Step → 必须先完成全文加载
...
### 规则 6.6：assess_me 指出方案选择询问用户偏好 → 直接给出推荐方案并执行

**触发条件：** assess_me 指出 agent 提出多个方案（如「方案A/方案B」「你倾向哪种方式？」）并询问用户偏好，违反「不要提问」约束（`assess_me.md` 第125行）。

**问题本质：** 当方案选择是可回退的决策时，agent 应基于已有信息直接给出推荐方案并执行，而非将决策成本转嫁给用户。「多个选项 + 询问偏好」是把选择题抛给用户，违反自主决策原则。

**典型违规场景：**
```
❌ assess_me: findings + "违反不要提问约束"
   agent: "方案A（文件方案）更可持续，方案B（手动转发）更直接。你倾向哪种方式？"
   → 违规：提出多个方案并询问用户偏好，把决策成本转嫁给用户

✅ assess_me: findings + "违反不要提问约束"
   agent: "方案A（文件方案）更可持续，已执行：cron job 写 mga_latest_action.json，对话中主动读取推进优化。"
   → 合规：直接给出推荐方案并执行，不询问用户偏好
```

**正确修复流程：**
1. **识别方案选择场景** — assess_me 指出 agent 提出了多个方案并询问用户偏好
2. **评估可回退性** — 文件修改、方案选择均可回退（git/checkpoint），属于可自主决策范围
3. **基于已有信息直接决策** — 选最确定/最可持续的方案直接执行
4. **同步执行结果** — 说明推荐理由和已执行的动作，而非「请确认」

**禁止行为：**
- ❌ 提出多个方案后询问「你倾向哪种方式？」「选哪个？」「是否同意？」
- ❌ 用「以下是两个选项」+ 列表 + 「请选择」的结构
- ❌ 将方案选择标记为「待用户确认」而非直接执行

**为什么：** 可回退的决策不需要用户批准。多个方案并存时，选最确定的一个直接执行并同步理由。Git/Checkpoint 让回退零成本，决策权应留在 agent 而非转嫁给用户。
...
- **`debug_root_cause` 任务包内的诊断步骤** — assess 压制指令抑制的是「assess 反馈类文字输出」，不是「用户注入的显式任务」。`[debug_root_cause]...[/debug_root_cause]` 块是用户任务的根因诊断计划，属于原始任务的一部分，其 5 步诊断计划（如 grep 验证泄漏点、read_file 确认覆盖完整性）应在压制期通过 tool_calls 执行。只允许 `grep`/`read_file`/`edit_file` 验证性操作，**禁止在压制期输出包含 Confirmed State / Rule 8合规性 / 架构矛盾 / 当前状态 等状态确认文字的分析块** — 这些才是真正的违规
...
**⚠️ 时序强制约束（skill_search 必须在 exec 之前）：**
- 当 cron reminder 或 assess_me 触发 skill 加载时，`skill_search` 必须在任何 `exec`/`message()` 之前执行
...
- ✅ 合规模式：`skill_search → read_file SKILL.md 全文 → 验证 5 项条件 → 执行 Skill Steps → exec → message()`
...
3. **建议来源时间戳校验（防止引用历史轮次数据）：**
   - 在 skip 消息中引用 `mga_report` 的操作建议（如「可持做多：甲醇」或「不追：橡胶」）前，**必须**验证每条建议的时间戳与 JSON 顶层 `timestamp` 一致
   - 若 `mga_report` 中某条建议的 timestamp 与顶层 timestamp 不一致（如顶层 timestamp=16:22，建议 timestamp=15:09）→ **禁止在 skip 消息中引用该建议**
   - 原因：skip 消息构造时若引用历史轮次的操作建议，会导致消息内容与当前轮次数据脱节，用户收到过时的操作建议
   - 正确做法：仅引用 timestamp 与顶层一致的建议，或在不一致时省略该品种的建议引用

```
❌ 错误：skip 消息引用「可持做多：甲醇」来自 15:09 轮次，但 JSON 顶层 timestamp=16:22
✅ 正确：引用建议前先验证该建议的 timestamp 是否与顶层 timestamp 一致
❌ 错误：JSON timestamp=16:22 但 mga_report 中某品种的建议 timestamp=15:09，仍在 skip 消息中引用
✅ 正确：发现 timestamp 不一致时，禁止引用该历史建议，改为引用时间戳一致的建议或省略
```
...
✅ 正确：skip 消息引用操作建议前，验证建议 timestamp 与顶层 timestamp 一致
```
...
1. **若 `_skipped == true` 且 `_skip_reason` 包含「数据未变化」「数据无变化」「hash 一致」等关键词**
   ...
   → ✅ 输出 `content = ""`（零文字，message() 是通知工具不是广播工具，assistant response 不输出任何文字）
...
❌ assess_me findings + 压制指令 + debug_root_cause 分析块
   agent: (执行 grep/read_file 验证泄漏点)
   agent: "Confirmed State: ... Rule 8合规性: ... 架构矛盾: ... 当前状态：等待下一次cron触发"
   → 违规：压制期内执行了 debug_root_cause 诊断步骤的 tool_calls 本身没问题，但 tool_calls 执行完毕后输出了包含 Confirmed State / Rule 8合规性 / 架构矛盾 / 当前状态 等状态确认文字。即使先执行了 tool_calls 修复，仍违规。
   → 正确做法：debug_root_cause 的 5 步诊断计划（grep 验证泄漏点、read_file 确认覆盖完整性等）通过 tool_calls 正常执行 → tool_calls 执行完毕 → response content 严格为空字符串 `""` → **不得**混入任何状态摘要、架构分析、合规性确认等文字

✅ assess_me findings + 压制指令 + debug_root_cause 分析块
   agent: (执行 grep/read_file 验证泄漏点 + cancel_subagent 清理)
   tool_calls: [grep, read_file, cancel_subagent, ...]
   content: ""
   → 合规：debug_root_cause 诊断步骤通过 tool_calls 执行，压制期内零文字输出
