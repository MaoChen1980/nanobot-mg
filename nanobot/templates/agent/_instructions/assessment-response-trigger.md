...
### 规则 6：assess_me 指出决策逻辑偏差时 → 先验证数据而非探索代码

**触发条件：** assess_me 的 `content` 描述了具体数据值和变化（如「苯乙烯 V/OI +0.28」「某指标从 A 变为 B，差值 > 阈值」），并指出 agent 的响应与数据不符（如「agent 未触发消息但数据已超阈值」）。

**核心区分：两类根因，两种验证路径：**

| assess_me 质疑类型 | 根因 | 验证路径 |
|-------------------|------|---------|
| **事实合规冲突**（Rule 1） | agent 引用的数据/代码与实际不符 | 读代码位置，验证代码是否真实存在 |
| **决策逻辑偏差**（本规则） | 数据正确但 agent 判断逻辑有误 | 对比前后轮次输出，验证数据是否真实变化 |

**动作：**
1. **第一步：验证数据真实性** — 对比 conversation 历史中前后轮次的实际输出数据
   - 若 assess_me 说「X 品种从 A 变为 B」→ 找到两轮输出，确认 A 和 B 是否真实
   - 若数据不符 → 这是 Rule 1（事实合规冲突），走 Rule 1 路径
   - 若数据相符 → 确认是决策逻辑问题，继续下一步
2. **第二步：确定修复方向**
   - 若问题是「数据正确但阈值设置不合理」→ 修正阈值逻辑
   - 若问题是「缺少关注品种白名单」→ 建立白名单
   - 若问题是「整体稳态判断掩盖了局部异常」→ 修正判断逻辑
3. **禁止：** 在未确认数据真实性的情况下，直接用 grep/read_file 探索代码——这混淆了两类任务（验证 vs 修复），会导致修复方向错误

**典型失败模式：** assess 指出「苯乙烯 V/OI +0.28 超阈值但 agent 未触发」→ agent 不验证数据直接 grep 代码 → 发现脚本有相关逻辑就认为代码没问题 → 跳过数据验证 → assess 再次指出相同问题

**正确路径：**
```
assess_me: "苯乙烯 V/OI +0.28 超阈值但零消息"
→ 验证：对比 22:00 和 22:30 输出中苯乙烯 V/OI
→ 若 0.95→1.23 属实 → 确认是 agent 决策逻辑问题
→ 确定修复方向：建立关注品种白名单 或 修正阈值判断
```
