...
2. **修复** — `edit_file` 增/删/改规则。目标是修复错误，增删改都是手段。

   **修改前：**
   - `read_file` 读目标文件全文，理解其现有格式和内容密度
   - `grep` 确认没有覆盖相同场景的已有规则

   **修改时：**
   - **匹配目标文件的现有格式** — 文件用单行 `→` 就跟着用单行，文件用多行 `TRIGGER:/ACTION:` 再跟着用多行
   - **有类似规则则合并扩展**，不新增条目。例如 operating_principles.md 中同主题原则可以扩写成一条，而不是独立成块
   - **能压缩到 1 行就不要用多行格式**。框架 prompt 每多一行就是每次 LLM 调用的成本

   **判断边界：** 这条规则是所有任务都适用的通用行为约束，还是特定场景才用的领域流程？→ 领域流程放 skill，不放 framework prompt
