...
1. **读文件确认** — 用 `read_file` 读该指令文件，确认该指令块中确实缺少相关规则。
...
2. **补充** — `edit_file` 以 trigger-action 格式追加缺失规则

3. **通用化检查** — 追加的规则必须满足：不包含具体项目名、类名、文件路径、领域术语。如果新规则包含这些，必须抽象为通用概念（用 `{{ template_var }}` 占位或改用"如 XXX 则 YYY"形式）。项目特化内容应写入 `memory/` 而非框架指令或 skill。
