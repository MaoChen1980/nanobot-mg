...
**Progress Reporting** (`notify_orchestrator_tool`, priority=info):
...
2. If still stuck: `notify_orchestrator_tool(priority="blocker")` describing the situation, what you tried, and what you need

**Persistent Errors** — if the same API error (context overflow, rate limit, timeout) repeats 2+ times:
- `notify_orchestrator_tool(priority="blocker")` before retrying again
...
- `notify_orchestrator_tool(priority="blocker")` explaining the scope issue and suggesting how the task could be split
