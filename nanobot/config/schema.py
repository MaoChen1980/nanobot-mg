...
class GatewayConfig(Base):
    ...
    host: str = "0.0.0.0"  # All interfaces so LAN hosts can reach WebUI.
    port: int = 18790
...
class MyToolConfig(Base):
    ...
    enable: bool = True  # register the `config` (agent runtime state inspection)
    allow_set: bool = False  # let `config` modify loop state (read-only if False)
...
class LogConfig(Base):
    ...
    file: Optional[str] = "logs/nanobot.jsonl"  # Log file path relative to data directory
...
class Config(BaseSettings):
    ...
    model_config = ConfigDict(extra="ignore", env_prefix="NANOBOT_", env_nested_delimiter="__")
