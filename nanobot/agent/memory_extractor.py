...
class MemoryExtractor:
    ...
    async def _consolidate_topic_content(self, rel_path: str) -> None:
        ...
        system_msg = (
            ...
            "## 第一步：内容类型诊断\n"
            "先阅读内容，判断它属于以下哪种类型，再选择对应的结构模板。如果不匹配任一类型，使用【兜底模板】。\n\n"
            "### 类型 1：技术文档 / 操作指南\n"
            "内容特征：涉及工具使用、配置参数、命令行操作、代码片段。\n"
            "结构模板：\n"
            "- 📦 功能/系统概述\n"
            "- 📐 实现方案与关键参数\n"
            "- ⚠️ 边界条件与避坑\n\n"
            "### 类型 2：项目进展 / 移植追踪\n"
            "内容特征：涉及模块移植状态、轮次进度、待办列表、已/未完成工作。\n"
            "结构模板：\n"
            "- 🎯 总体状态（一句话总结）\n"
            "- ✅ 已完成/已移植\n"
            "- 📋 待完成/待移植\n"
            ...
            "- ✅ 解决方案\n\n"
            "### 类型 4：经验教训 / 行为准则\n"
            "内容特征：涉及踩坑总结、做事方法、执行策略、不应做什么。\n"
            "结构模板：\n"
            "- ⚡ 核心原则\n"
            "- ✅ 推荐做法\n"
            "- ❌ 应避免\n\n"
            "### 类型 5：兜底模板（上述均不匹配时使用）\n"
            "自行通过分析文本内在逻辑（因果、时序、优先级）归纳分级标题。\n"
            "结构模板：\n"
            "- 📌 核心摘要\n"
            "- 🧱 内容详情（## / ### 层级）\n"
            "- 💡 关键要点\n\n"
            "## 第二步：重组规则\n"
            "1. 合并重复或高度重叠的条目，保留信息最完整的那条\n"
            "2. **必须保留所有 HTML 注释元数据** — 每个段落后的 <!--ts:timestamp--><!--pinned--><!--recent--> 不能丢失或改变\n"
            "3. 保持原始措辞，不要润色或重写技术内容\n"
