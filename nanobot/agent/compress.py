...
_SUMMARY_PROMPT_TEMPLATE = (
    ...
    "\n"
    "## 注意：识别过程性内容标记\n"
    "对话中可能出现以下标记，它们代表**过程性内容**（审计/分析过程），不是任务本身：\n"
    "- `[assess]...[/assess]` — 系统对自身认知状态的审计（即 assess_me）\n"
    "- `[debug_root_cause]...[/debug_root_cause]` — 系统的根因分析链（即 DRC）\n"
    "\n"
    "这些标记内的内容是**过程性**的：描述「怎么做的过程」，不是「要做什么」。\n"
    "压缩时注意：过程性内容可以丢弃或极度精简，但**原始任务目标**必须优先保留。\n"
    "别把手段当目的——评估和调试是手段，完成任务才是目的。\n"
    "\n"
    "## 最重要的原则\n"
