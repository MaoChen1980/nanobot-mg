...
_SUMMARY_PROMPT_TEMPLATE = (
    "你正在压缩旧对话 turns。目标是生成一份**面向未来的摘要**——"
    "告诉未来的自己「现在在做什么、怎么走到这里的、接下来该干嘛」。\n"
    "\n"
    "后面附上了保留的对话（最近几轮），它代表当前的进行状态。\n"
    "请参考它来判断旧对话中哪些信息对未来还有价值。\n"
    "\n"
    "## 摘要结构（按顺序输出）\n"
    "\n"
    "### 目标\n"
    "当前任务的根目标是什么。这是不变的部分——每次压缩都需明确保留。\n"
    "\n"
    "### 当前状态\n"
    "现在进行到哪一步了、有什么产出/结果、阻塞在哪。\n"
    "保留当前状态的关键细节：比如已确认的关键文件路径、配置值、\n"
    "API 签名等无法通过 conversation_search_tool 找回的实时信息。\n"
    "\n"
    "### 到达路径\n"
    "怎么从起点走到这里的——尝试了哪些方向、哪些成功了、哪些失败了。\n"
    "把试错链压缩成结论，不要罗列中间步骤。\n"
    "示例：「已确认方案 A 可行」「方案 B 因 XXX 原因失败，已排除」\n"
    "\n"
    "### 下一步\n"
    "接下来应该做什么。基于当前状态和目标，明确下一步行动。\n"
    "\n"
    "## 哪些可以丢弃\n"
    "以下信息**不需要**保留——未来 LLM 可以通过工具调用重新获得：\n"
    "- 文件路径、目录结构、代码位置 → 用 read_file_tool / glob_tool 重新查\n"
    "- 工具调用的原始输出 → LLM 可以重新执行获取\n"
    "- 详细试错过程 → 只需保留结论（哪个方案可行/不可行）\n"
    "- 已经过时的中间值/错误 → 已被后续修复覆盖\n"
    "- 寒暄、确认性质的对话\n"
    "\n"
    "## 注意：过程性内容标记\n"
    "对话中可能出现以下标记，它们代表**过程性内容**，不是任务本身：\n"
    "- `[assess]...[/assess]` — 系统对自身认知状态的审计\n"
    "- `[debug_root_cause]...[/debug_root_cause]` — 系统的根因分析链\n"
    "这些可以丢弃或极度精简。目标才最重要。\n"
    "\n"
    "## 输出要求\n"
    "按上述四个章节输出，简洁的要点列表，不要按 turn 顺序罗列。\n"
    "用自然语言。不要包含 XML、代码、或工具调用格式。\n"
    "\n"
    "{previous_summary}"
    "以下是即将被裁剪的旧对话：\n\n"
    "{turns_text}\n\n"
