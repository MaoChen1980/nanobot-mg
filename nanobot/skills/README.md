...
`grep_tool` / `glob_tool` tools to narrow the search space before loading full files.
Use `grep_tool(output_mode="count")` / `files_with_matches` for broad searches first,
use `head_limit` / `offset` to page through large result sets,
and `glob_tool(entry_type="dirs")` when discovering directory structure matters.
...
| `self_tool` | Check and set the agent's own runtime state |
